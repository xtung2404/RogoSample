...
class PairedRfDeviceSpinnerAdapter(context: Context, deviceList: List<IoTPairedRfDevice>)
    : ArrayAdapter<IoTPairedRfDevice>(context, 0, deviceList){
    ...
    private fun getCustomView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        getItem(position)?.let {
            textViewName.text = SmartSdk.getProductModel(it.productId).name?: "unknown"
        }
